...
- **Masaüstü/Web:** Geniş ekran alanları avantajıyla çok sütunlu gelişmiş veri tabloları, akıllı filtreleme (Autocomplete) çubukları, split-screen (bölünmüş ekran) PDF editörü ve yan paneller aktif olur.
...
- **`http` & `shared_preferences`:** Sunucu ile JWT token tabanlı güvenli iletişim ve yerel oturum yönetimi.
...
- `screens/` -> Görsel ekranların ana sayfaları (Teklifler, Müşteriler, Şablon Düzenleyici vb.).
- `widgets/` -> Performans optimizasyonu için parçalanmış, responsive duyarlı reusable (tekrar kullanılabilir) alt bileşenler.
- `services/` -> API servis katmanı; HTTP protokolleri ve MultipartRequest işlemleri üzerinden sunucuyla iletişimi sağlar.
- `models/` -> Gelen karmaşık JSON verilerini Dart nesnelerine dönüştüren sistem modelleri.
